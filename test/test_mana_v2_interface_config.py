# coding: utf-8

"""
    Graphiant APIs

    Graphiant API documentation.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.mana_v2_interface_config import ManaV2InterfaceConfig

class TestManaV2InterfaceConfig(unittest.TestCase):
    """ManaV2InterfaceConfig unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ManaV2InterfaceConfig:
        """Test ManaV2InterfaceConfig
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ManaV2InterfaceConfig`
        """
        model = ManaV2InterfaceConfig()
        if include_optional:
            return ManaV2InterfaceConfig(
                admin_status = True,
                alias = 'example string',
                circuit = 'example string',
                description = 'example string',
                duplex = 'example string',
                ipsec = graphiant_sdk.models.mana_v2_interface_i_psec_config.manaV2InterfaceIPsecConfig(
                    anti_replay_window_size = 123, 
                    dh_group = 'ENUM_VALUE', 
                    dpd_interval = 123, 
                    encryption_alg = 'ENUM_VALUE', 
                    esn = True, 
                    ike_integrity = 'ENUM_VALUE', 
                    initiator = True, 
                    ipsec_encryption_alg = 'ENUM_VALUE', 
                    ipsec_integrity = 'ENUM_VALUE', 
                    label = 'ENUM_VALUE', 
                    local_address = 'example string', 
                    local_circuit = 'example string', 
                    local_ike_peer_identity = 'example string', 
                    perfect_forward_secrecy = 'ENUM_VALUE', 
                    preshared_key = 'example string', 
                    reauth_interval = 123, 
                    rekey_interval = 123, 
                    remote_address = 'example string', 
                    remote_ike_peer_identity = 'example string', ),
                ipv4 = graphiant_sdk.models.mana_v2_interface_ip_config.manaV2InterfaceIpConfig(
                    address = graphiant_sdk.models.mana_v2_nullable_address.manaV2NullableAddress(), 
                    dhcp = graphiant_sdk.models.mana_v2_interface_dhcp_config.manaV2InterfaceDhcpConfig(
                        dhcp_client = True, 
                        dhcp_relay = graphiant_sdk.models.mana_v2_dhcp_relay_config.manaV2DhcpRelayConfig(
                            relay_servers = [
                                'example string'
                                ], ), ), 
                    gw = graphiant_sdk.models.mana_v2_nullable_gateway_config.manaV2NullableGatewayConfig(), 
                    vrrp = graphiant_sdk.models.mana_v2_nullable_vrrp_group_config.manaV2NullableVrrpGroupConfig(
                        group = graphiant_sdk.models.mana_v2_vrrp_group_config.manaV2VrrpGroupConfig(
                            accept_mode = True, 
                            allow_inter_operability = True, 
                            description = 'example string', 
                            enabled = True, 
                            preempt = True, 
                            priority = 123, 
                            tracked_interfaces = [
                                graphiant_sdk.models.mana_v2_nullable_interface_priority_decrement.manaV2NullableInterfacePriorityDecrement(
                                    interface = 'example string', 
                                    priority_decrement = 123, )
                                ], 
                            virtual_ip = 'example string', 
                            virtual_router_id = 123, ), ), ),
                ipv6 = graphiant_sdk.models.mana_v2_interface_ip_config.manaV2InterfaceIpConfig(
                    address = graphiant_sdk.models.mana_v2_nullable_address.manaV2NullableAddress(), 
                    dhcp = graphiant_sdk.models.mana_v2_interface_dhcp_config.manaV2InterfaceDhcpConfig(
                        dhcp_client = True, 
                        dhcp_relay = graphiant_sdk.models.mana_v2_dhcp_relay_config.manaV2DhcpRelayConfig(
                            relay_servers = [
                                'example string'
                                ], ), ), 
                    gw = graphiant_sdk.models.mana_v2_nullable_gateway_config.manaV2NullableGatewayConfig(), 
                    vrrp = graphiant_sdk.models.mana_v2_nullable_vrrp_group_config.manaV2NullableVrrpGroupConfig(
                        group = graphiant_sdk.models.mana_v2_vrrp_group_config.manaV2VrrpGroupConfig(
                            accept_mode = True, 
                            allow_inter_operability = True, 
                            description = 'example string', 
                            enabled = True, 
                            preempt = True, 
                            priority = 123, 
                            tracked_interfaces = [
                                graphiant_sdk.models.mana_v2_nullable_interface_priority_decrement.manaV2NullableInterfacePriorityDecrement(
                                    interface = 'example string', 
                                    priority_decrement = 123, )
                                ], 
                            virtual_ip = 'example string', 
                            virtual_router_id = 123, ), ), ),
                lan = 'example string',
                lldp_enabled = True,
                loopback = True,
                macsec = graphiant_sdk.models.mana_v2_nullable_ma_csec_configuration.manaV2NullableMaCsecConfiguration(),
                max_transmission_unit = 123,
                security_zone = 'example string',
                speed = 1234567891011,
                subinterfaces = {
                    'key' : graphiant_sdk.models.mana_v2_nullable_interface_vlan_config.manaV2NullableInterfaceVlanConfig(
                        interface = graphiant_sdk.models.mana_v2_interface_vlan_config.manaV2InterfaceVlanConfig(
                            admin_status = True, 
                            alias = 'example string', 
                            circuit = 'example string', 
                            description = 'example string', 
                            ipv4 = graphiant_sdk.models.mana_v2_interface_ip_config.manaV2InterfaceIpConfig(
                                address = graphiant_sdk.models.mana_v2_nullable_address.manaV2NullableAddress(), 
                                dhcp = graphiant_sdk.models.mana_v2_interface_dhcp_config.manaV2InterfaceDhcpConfig(
                                    dhcp_client = True, 
                                    dhcp_relay = graphiant_sdk.models.mana_v2_dhcp_relay_config.manaV2DhcpRelayConfig(
                                        relay_servers = [
                                            'example string'
                                            ], ), ), 
                                gw = graphiant_sdk.models.mana_v2_nullable_gateway_config.manaV2NullableGatewayConfig(), 
                                vrrp = graphiant_sdk.models.mana_v2_nullable_vrrp_group_config.manaV2NullableVrrpGroupConfig(
                                    group = graphiant_sdk.models.mana_v2_vrrp_group_config.manaV2VrrpGroupConfig(
                                        accept_mode = True, 
                                        allow_inter_operability = True, 
                                        description = 'example string', 
                                        enabled = True, 
                                        preempt = True, 
                                        priority = 123, 
                                        tracked_interfaces = [
                                            graphiant_sdk.models.mana_v2_nullable_interface_priority_decrement.manaV2NullableInterfacePriorityDecrement(
                                                priority_decrement = 123, )
                                            ], 
                                        virtual_ip = 'example string', 
                                        virtual_router_id = 123, ), ), ), 
                            ipv6 = graphiant_sdk.models.mana_v2_interface_ip_config.manaV2InterfaceIpConfig(), 
                            lan = 'example string', 
                            lldp_enabled = True, 
                            max_transmission_unit = 123, 
                            security_zone = 'example string', 
                            tcp_mss = 123, 
                            tcp_mss_v4 = 123, 
                            tcp_mss_v6 = 123, 
                            v4_tcp_mss = graphiant_sdk.models.mana_v2_nullable_tcp_mss_v4.manaV2NullableTcpMssV4(
                                tcp_mss_v4 = 123, ), 
                            v6_tcp_mss = graphiant_sdk.models.mana_v2_nullable_tcp_mss_v6.manaV2NullableTcpMssV6(
                                tcp_mss_v6 = 123, ), 
                            vlan = 123, 
                            vrrp = graphiant_sdk.models.mana_v2_nullable_vrrp_group_config.manaV2NullableVrrpGroupConfig(), ), )
                    },
                tcp_mss = 123,
                tcp_mss_v4 = 123,
                tcp_mss_v6 = 123,
                v4_tcp_mss = graphiant_sdk.models.mana_v2_nullable_tcp_mss_v4.manaV2NullableTcpMssV4(
                    tcp_mss_v4 = 123, ),
                v6_tcp_mss = graphiant_sdk.models.mana_v2_nullable_tcp_mss_v6.manaV2NullableTcpMssV6(
                    tcp_mss_v6 = 123, )
            )
        else:
            return ManaV2InterfaceConfig(
        )
        """

    def testManaV2InterfaceConfig(self):
        """Test ManaV2InterfaceConfig"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
