# coding: utf-8

"""
    Graphiant APIs

    Graphiant API documentation.

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from graphiant_sdk.models.v1_users_id_groups_root_get_response import V1UsersIdGroupsRootGetResponse

class TestV1UsersIdGroupsRootGetResponse(unittest.TestCase):
    """V1UsersIdGroupsRootGetResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> V1UsersIdGroupsRootGetResponse:
        """Test V1UsersIdGroupsRootGetResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `V1UsersIdGroupsRootGetResponse`
        """
        model = V1UsersIdGroupsRootGetResponse()
        if include_optional:
            return V1UsersIdGroupsRootGetResponse(
                groups = [
                    graphiant_sdk.models.iam_group.iamGroup(
                        description = 'example string', 
                        enterprise_ids = [
                            1234567891011
                            ], 
                        enterprise_permissions = {
                            'key' : graphiant_sdk.models.iam_enterprise_permissions.iamEnterprisePermissions(
                                group_type = 'ENUM_VALUE', 
                                permissions = graphiant_sdk.models.common_permissions.commonPermissions(
                                    asset_manager = 'ENUM_VALUE', 
                                    b2b = 'ENUM_VALUE', 
                                    b2b_security_profile_external = 'ENUM_VALUE', 
                                    billing_and_invoicing = 'ENUM_VALUE', 
                                    compliance = 'ENUM_VALUE', 
                                    developer_tools = 'ENUM_VALUE', 
                                    gateway = 'ENUM_VALUE', 
                                    global_services = 'ENUM_VALUE', 
                                    insights = 'ENUM_VALUE', 
                                    licensing = 'ENUM_VALUE', 
                                    logs = 'ENUM_VALUE', 
                                    monitoring_and_troubleshooting = 'ENUM_VALUE', 
                                    network_configuration = 'ENUM_VALUE', 
                                    order_status = 'ENUM_VALUE', 
                                    reports = 'ENUM_VALUE', 
                                    safety_and_security = 'ENUM_VALUE', 
                                    service_policies = 'ENUM_VALUE', 
                                    support = 'ENUM_VALUE', 
                                    user_and_tenant_management = 'ENUM_VALUE', ), )
                            }, 
                        group_type = 'ENUM_VALUE', 
                        id = 'example string', 
                        name = 'example string', 
                        permissions = graphiant_sdk.models.common_permissions.commonPermissions(
                            asset_manager = 'ENUM_VALUE', 
                            b2b = 'ENUM_VALUE', 
                            b2b_security_profile_external = 'ENUM_VALUE', 
                            billing_and_invoicing = 'ENUM_VALUE', 
                            compliance = 'ENUM_VALUE', 
                            developer_tools = 'ENUM_VALUE', 
                            gateway = 'ENUM_VALUE', 
                            global_services = 'ENUM_VALUE', 
                            insights = 'ENUM_VALUE', 
                            licensing = 'ENUM_VALUE', 
                            logs = 'ENUM_VALUE', 
                            monitoring_and_troubleshooting = 'ENUM_VALUE', 
                            network_configuration = 'ENUM_VALUE', 
                            order_status = 'ENUM_VALUE', 
                            reports = 'ENUM_VALUE', 
                            safety_and_security = 'ENUM_VALUE', 
                            service_policies = 'ENUM_VALUE', 
                            support = 'ENUM_VALUE', 
                            user_and_tenant_management = 'ENUM_VALUE', ), 
                        time_window_end = 1234567891011, 
                        time_window_start = 1234567891011, )
                    ]
            )
        else:
            return V1UsersIdGroupsRootGetResponse(
        )
        """

    def testV1UsersIdGroupsRootGetResponse(self):
        """Test V1UsersIdGroupsRootGetResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
